...
import com.piero.backend.chat.app.auth.service.JwtService;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        final Optional<Usuario> usuarioEncontrado = usuarioRepository.findByCorreo(userDetails.getUsername());
        if (usuarioEncontrado.isEmpty()) {
            filterChain.doFilter(request, response);
            return;
        }
        final boolean isTokenValid = jwtService.isTokenValid(jwtToken, usuarioEncontrado.get());
        if (!isTokenValid) {
            return;
        }
