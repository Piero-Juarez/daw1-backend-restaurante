...
public class CategoriaServiceImpl implements CategoriaService {
    ...
    public CategoriaDTOResponse guardarCategoria(CategoriaDTORequest categoriaRequest) {
        if (categoriaRepository.existsCategoriaByNombre(categoriaRequest.nombre())) {
            throw new SystemError("El nombre "+categoriaRequest.nombre() + " de la categoría ya existe. Debe ingresar uno nuevo.");
        }
    ...
    public CategoriaDTOResponse actualizarCategoria(Short id, CategoriaDTORequest categoriaRequest) {
        Categoria categoria = categoriaRepository.findById(id).orElseThrow(() -> new SystemError("Categoria no encontrada con id: " + id));
    ...
    public CategoriaDTOResponse obtenerCategoriaPorId(Short id) {
        return categoriaMapper.toDtoResponse(categoriaRepository.findById(id).orElseThrow(() -> new SystemError("Categoria no encontrada con id: " + id))) ;
    }

    @Override
    public void eliminarCategoria(Short idCategoria) {
        Categoria categoria = categoriaRepository.findById(idCategoria).orElseThrow(() -> new SystemError("Categoria no encontrada con id: " + idCategoria));
